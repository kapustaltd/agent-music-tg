...
// Every authenticated API request upserts its caller, purely to bump last_seen
// — including each Range request while a track streams. Throttle those writes
// so a burst from one active chat doesn't turn into a SELECT plus a WAL commit
// per request; identity changes (username/first_name) land at the next window
// instead of immediately, which is well within the tolerance for a display
// name. Keyed per-db (not just chatId) so separate AppDb instances — as in
// tests, each opening its own :memory: db and reusing the same chat ids —
// never share throttle state.
const LAST_SEEN_THROTTLE_MS = 60_000;
const LAST_SEEN_MAX_CHATS = 10_000;
interface SeenEntry {
  at: number;
  /** Identity last written for this chat; a change bypasses the throttle. */
  identity: string;
}
const recentlySeenByDb = new WeakMap<AppDb, Map<number, SeenEntry>>();
...
export function upsertUser(
  ...
  const now = Date.now();
  const identity = `${username ?? ""}\u001f${firstName ?? ""}`;
  // Being in the map means this chat was already upserted, so it is never new.
  // A changed username/first_name still writes through immediately — only
  // repeat calls carrying the same identity are skipped.
  if (last !== undefined && last.identity === identity && now - last.at < LAST_SEEN_THROTTLE_MS) {
    return false;
  }
  ...
  recentlySeen.set(chatId, { at: now, identity });
